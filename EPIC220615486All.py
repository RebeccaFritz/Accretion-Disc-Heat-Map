...
def velfinal(particleidx, otheridx):
    ...
    v1diff = v1f - v1i # difference between inital and final velocites for particle 1
    v2diff = v2f - v2i # difference between inital and final velocites for particle 2
    
    particle_list[particleidx].vel = v1i + v1diff*0.1#0.1
    particle_list[otheridx].vel = v2i + v2diff*0.1#0.1   
...
# a function to run the Runge-Kutta alg on particles (the original from Dr. Olenick)      
## void --> void
def rkParticlesOlen():
    for particle in particle_list:  
...
# a function to run the Runge-Kutta alg on particles (with viscosity)     
## void --> void
def rkParticlesOlenVis():
    for particle in particle_list:  
        alpha = 0.5e-10
        beta = 0.5e-10    
        rA = mag(particle.pos - starA.pos)
        ...
        if rA > L1/100 and rB > L2/100 and particle.visible: 
            ...
            acc_p = G*starA.mass*(starA.pos - particle.pos)/rA**3 + G*starB.mass*(starB.pos - particle.pos)/rB**3
            viscosity_1 = +alpha*particle.vel - beta*particle.vel*mag2(particle.vel)/mag(particle.vel) #-alpha*velocity + beta*velocity*velocity**2
            k1v = hp*(acc_p + viscosity_1)
            k1x = hp*particle.vel
            
            acc_2 = (G*starA.mass*(starA.pos - (particle.pos+k1x/2.0))/rA**3 + G*starB.mass*(starB.pos - (particle.pos+k1x/2.0))/rB**3)
            viscosity_2 = +alpha*(particle.vel + k1v/2.0) - beta*(particle.vel + k1v/2.0)*mag2(particle.vel + k1v/2.0)/mag(particle.vel + k1v/2.0)
            k2v = hp*(acc_2 + viscosity_2)
            k2x = hp*(particle.vel + k1v/2.0)
            
            acc_3 = (G*starA.mass*(starA.pos - (particle.pos+k2x/2.0))/rA**3 + G*starB.mass*(starB.pos - (particle.pos+k2x/2.0))/rB**3)
            viscosity_3 = +alpha*(particle.vel + k2v/2.0) - beta*(particle.vel + k2v/2.0)*mag2(particle.vel + k2v/2.0)/mag(particle.vel + k2v/2.0)
            k3v = hp*(acc_3 + viscosity_3)
            k3x = hp*(particle.vel + k2v/2.0)
            
            acc_4 = (G*starA.mass*(starA.pos - (particle.pos+k3x/2.0))/rA**3 + G*starB.mass*(starB.pos - (particle.pos+k3x/2.0))/rB**3) 
            viscosity_4 = +alpha*(particle.vel + k3v) - beta*(particle.vel + k3v/2.0)*mag2(particle.vel + k3v/2.0)/mag(particle.vel + k3v/2.0)
            k4v = hp*(acc_4 + viscosity_4)            
...
def rkParticlesOlenGrav():
    for particle in particle_list:  
        ...
        if rA > L1/100 and rB > L2/100 and particle.visible: 
            ...
            viscosity_1 = vec(0,0,0)#+alpha*particle.vel - beta*particle.vel*mag2(particle.vel)/mag(particle.vel) #-alpha*velocity + beta*velocity*velocity**2
            k1v = hp*(acc_p + viscosity_1)
            k1x = hp*particle.vel
            ...
            viscosity_2 = vec(0,0,0)#+alpha*(particle.vel + k1v/2.0) - beta*(particle.vel + k1v/2.0)*mag2(particle.vel + k1v/2.0)/mag(particle.vel + k1v/2.0)
            k2v = hp*(acc_2 + viscosity_2)
            k2x = hp*(particle.vel + k1v/2.0)
            ...
            viscosity_3 = vec(0,0,0)#+alpha*(particle.vel + k2v/2.0) - beta*(particle.vel + k2v/2.0)*mag2(particle.vel + k2v/2.0)/mag(particle.vel + k2v/2.0)
            k3v = hp*(acc_3 + viscosity_3)
            k3x = hp*(particle.vel + k2v/2.0)
            ...
            viscosity_4 = vec(0,0,0)#+alpha*(particle.vel + k3v) - beta*(particle.vel + k3v/2.0)*mag2(particle.vel + k3v/2.0)/mag(particle.vel + k3v/2.0)
            k4v = hp*(acc_4 + viscosity_4)            
            k4x = hp*(particle.vel + k3v)
...
#    ### calculating and printing the period
#    r = mag(starA.pos - starB.pos)
#    
...
#    a = (rmin + rmax)/2
#    
#    P_calc = sqrt(4*pi**2*a**3/(G*(starA.mass + starB.mass))) /86400 # calculated period for both orbits converted to days
    
#    print('simulation Period: {0:6.6f} days / actual period: {1} days'.format(P_calc, P_days))
    ###
    ...
    ############
#       #get a maximum and minimum velocity of particle[0] during the period of graphing
#       #so those values can be used to calculate temperature ranges for coloring purposes
#        if mag(particle_list[0].vel) > vmax:
#            vmax = mag(particle_list[0].vel)
#        if mag(particle_list[0].vel) < vmin:
#            vmin = mag(particle_list[0].vel)
#        
#    elif t>15000 and t<15002:
#        print('vmin: {0}, vmax {1}, vavg {2}'.format(vmin, vmax))
